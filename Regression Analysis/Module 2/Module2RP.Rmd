...
knitr::opts_knit$set(root.dir = normalizePath("C:/Users/randa/Dropbox/Masters/Autumn/TBANLT 540 Applied Regression Analysis/Module 2"))
```
...
```{r Data}
MRLabData <- read_excel("98312_Redfin_2021.xlsx")
summary(MRLabData)
```
...
```{r scatterplots}
ggplot(data=MRLabData, aes(x=BEDS, y=PRICE)) + geom_point(size=1)
ggplot(data=MRLabData, aes(x=BATHS, y=PRICE)) + geom_point(size=1)
ggplot(data=MRLabData, aes(x=`SQUARE FEET`, y=PRICE)) + geom_point(size=1)
ggplot(data=MRLabData, aes(x=`YEAR BUILT`, y=PRICE)) + geom_point(size=1) + xlim(1900, 2022) 
ggplot(data=MRLabData, aes(x=`LOT SIZE`, y=PRICE)) + geom_point(size=1) + xlim(0, 28000)
...
ggplot(data=MRLabData, aes(x=`YEAR BUILT`, y=PRICE)) + geom_point(size=2)+geom_smooth() + geom_point(size=1) + xlim(1900, 2022) 
ggplot(data=MRLabData, aes(x=`LOT SIZE`, y=PRICE)) + geom_point(size=2)+geom_smooth() + geom_point(size=1) + xlim(400, 28000)
...
```{r Regression2}
HouseFit2<-lm(PRICE~BATHS+BEDS+`SQUARE FEET`+`LOT SIZE`, data=MRLAB3)
summary(HouseFit2)
```
...
HouseFit5<-lm(PRICE~`SQUARE FEET`+ `YEAR BUILT`, data=MRLAB3)
summary(HouseFit5)
...
ggplot(data=MRLAB3, aes(x=`S

```QUARE FEET`, y=Price.stdres2))+geom_point()
...
HouseFitlog<-lm(LOGPRICE~LOGBath+LOGSqFoot+LOGLotSize+LOGYearBuilt, data=MRLAB3)
summary(HouseFitlog)
```
